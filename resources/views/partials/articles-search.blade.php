<div class="articles-list-container">
	<div class="articles-list header card pt-3 pb-2 px-3 mx-lg-5">
		@if($search_articles->count() > 0)
			<i class="fas fa-tag fa-2x"></i><span">Showing posts {{$word}} <span class="articles-list category">{{$search}}</span>.</span>
		@else 
			<i class="fas fa-tag fa-2x"></i><span">No posts matching query <span class="articles-list category">{{$search}}</span>.</span>
		@endif
	</div>
