<div class="news business">
	<div class="news business common header">
		<a href="" class="text-uppercase header-title px-2 py-1"><i class="fas fa-rss-square"></i> business</a>
		<a href="" class="more px-1 my-2 text-uppercase">more <i class="fas fa-angle-right"></i></a>
	</div>
	<div class="news business items-container">
		<div class="news business common news-item large p-3">
			<div class="news-item common inner-container">
				<div class="news-item common image" style="background-image:url(/images/music1.jpg)"></div>
				<div class="news-item common date px-1 ml-3">
					<span class="news-item date day">22</span>
					<span class="news-item date month">Feb</span>
					<span class="news-item date year">2018</span>
				</div>
				<a href="" class="news-item common category px-1 ml-3">Entertainement</a>
				<div class="news-item common title ml-3 mb-2"><a href="">Lorem ipsum dolor sit amet</a></div>
			</div>
			<div class="news-item common preview text">Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam,
			quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo
			consequat.</div>
			<button class=" news-item common btn btn-dark">Read More <i class="fas fa-angle-double-right"></i></button>
		</div>
		
	</div>
</div>